  ...
  )
{
  ...
  return MmSaveStateReadRegisterByIndex (CpuIndex,
           MmSaveStateGetRegisterIndex (Register, AMD_MM_SAVE_STATE_REGISTER_MAX_INDEX), Width, Buffer);
}
  ...
  )
{
  ...
  //
  if (NeedSysCall () == TRUE){
    *(UINT32*)Buffer = (UINT32) SysCall (SMM_SC_SVST_READ, CpuIndex, (UINTN) ((UINT8 *)gMmst->CpuSaveState[CpuIndex]
  ...
  if (Width > 4) {
    ...
    //
    if (NeedSysCall () == TRUE){
      *((UINT32*)Buffer + 1) = (UINT32) SysCall (SMM_SC_SVST_READ, CpuIndex,
        (UINTN) ((UINT8 *)gMmst->CpuSaveState[CpuIndex] + mCpuWidthOffset[RegisterIndex].Offset64Hi), Width - 4);
    } else {
